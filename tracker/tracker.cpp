                    ...
                    else
                    {
                        ...
                        if (strncmp(status, "Successful Download", strlen("Successful Download")) == 0)
                        {
                            ...
                            if (torrent.find(fileSha) != torrent.end())
                            {
                                ...
                                {
                                    if (userChunks.find(chunkSha) != userChunks.end())
                                    {
                                        if (find(userChunks[chunkSha].begin(), userChunks[chunkSha].end(), currUser->userId) == userChunks[chunkSha].end())
                                        {
                                            userChunks[chunkSha].push_back(currUser->userId);
                                        }
                                    }
                                }
                            }
                            ...
                            {
                                const string &chunkSha = pair.first;
                                const vector<string> &users = pair.second;

                                cout << "Chunk SHA: " << chunkSha << " -> Users: ";
