...
//     // stringstream ss(response);
//     // string filePath, fileSha;

//     // // Get filePath and fileSHA
//     // getline(ss, filePath, ':');
//     // getline(ss, fileSha, ':');

//     // // Process chunks
//     // string chunkShaBlock;
//     // while (getline(ss, chunkShaBlock, '&'))
//     // {
//     //     stringstream chunkStream(chunkShaBlock);
//     //     string chunkShaWithNumber;

//     //     // Read chunkSha and chunkNumber
//     //     if (getline(chunkStream, chunkShaWithNumber, ':') && !chunkShaWithNumber.empty())
//     //     {
//     //         // Split chunkShaWithNumber into chunkSha and chunkNumber
//     //         size_t commaPos = chunkShaWithNumber.find(',');
//     //         if (commaPos == string::npos)
//     //             continue; // invalid format

//     //         string chunkSha = chunkShaWithNumber.substr(0, commaPos);
//     //         int chunkNumber = stoi(chunkShaWithNumber.substr(commaPos + 1));

//     //         vector<UserDetails> userInfo;
//     //         string userBlock;

//     //         // Read user details after the colon
//     //         while (getline(chunkStream, userBlock, ':'))
//     //         {
//     //             // Users are separated by semicolon
//     //             stringstream userStream(userBlock);
//     //             string ip, port;

//     //             // Extract users from userBlock
//     //             size_t pos = 0;
//     //             while (getline(userStream, ip, ';') && getline(userStream, port, ';'))
//     //             {
...
//             // Read user details after the comma
//             while (getline(chunkStream, userBlock, ':'))
//             {
...
//                 while (getline(userStream, userDetail, ';'))
//                 {
//                     // Split userDetail into userID, IP, and port
//                     string userId, ip, port;

//                     size_t firstSemicolon = userDetail.find(';');
//                     if (firstSemicolon == string::npos)
//                         continue;

//                     userId = userDetail.substr(0, firstSemicolon); // User ID

//                     size_t secondSemicolon = userDetail.find(';', firstSemicolon + 1);
//                     if (secondSemicolon == string::npos)
//                         continue;

//                     ip = userDetail.substr(firstSemicolon + 1, secondSemicolon - firstSemicolon - 1); // IP
//                     port = userDetail.substr(secondSemicolon + 1);                                    // Port

//                     // Store the user details if valid
//                     if (!userId.empty() && !ip.empty() && !port.empty())
//                     {
//                         UserDetails userDetails = {ip, port};
//                         userInfo.push_back(userDetails);
//                     }
...
//     //     string temp = "Download:" + to_string(chunkNumber) + ":" + filePath + ":" + chunkHash;
//     //     char *message = new char[temp.size() + 1];
//     //     strcpy(message, temp.c_str());
...
//         // Randomly select a user for downloading the chunk
//         int randomIndex = rand() % users.size();
//         string userIp = users[randomIndex].ipaddress;
//         string port = users[randomIndex].port;
...
//         struct sockaddr_in seederData;
//         seederData.sin_family = AF_INET;
//         seederData.sin_addr.s_addr = inet_addr(userIp.c_str());
//         uint16_t PORT = static_cast<uint16_t>(strtoul(port.c_str(), NULL, 10));
//         seederData.sin_port = htons(PORT);
...
//         struct timeval timeout;
//         timeout.tv_sec = 5; // Set timeout for connection
//         timeout.tv_usec = 0;
...
//     if (fileSha == receivedSha)
//     {
//         cout << "File downloaded and validated successfully.\n";
//         close(newFileFd);
//         return true;
//     }
//     else
//     {
//         cout << "SHA mismatch. File may be corrupted.\n";
//         close(newFileFd);
//         return false;
//     }
// }
